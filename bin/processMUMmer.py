...
mummer_dir = os.path.normpath(os.path.abspath(sys.argv[1]))

# Set filtering criteria
min_cov = float(85)
min_iden = float(99)
min_len = int(500)
density_windows = [1000,125,15]
max_snps = [3,2,1]
...
#min_iden = float(sys.argv[3])
#min_len = int(sys.argv[4])
#density_windows = [int(x) for x in sys.argv[5].split(",")]
#max_snps = [int(x) for x in sys.argv[6].split(",")]
#qc_density =  ",".join([str(x) for x in density_windows])
#qc_maxsnps =  ",".join([str(x) for x in max_snps])
#assert len(density_windows) == len(max_snps)
...
mummer_reports = glob(os.path.join(mummer_dir, '*.report'))

# Read the top line of the report, and split the text by " "
for report in mummer_reports:
    
    report_header = os.path.splitext(os.path.basename(report))[0]
    reference_fasta,query_fasta = open(report).readline().split(" ")
    reference_fasta = reference_fasta.strip()
    query_fasta = query_fasta.strip()
    if os.path.exists(reference_fasta) & os.path.exists(query_fasta):
        query = os.path.splitext(os.path.basename(query_fasta))[0]
        reference = os.path.splitext(os.path.basename(reference_fasta))[0]
        ...
        report_id = f"{query}__vs__{reference}"
        snpdiffs_file = f"{mummer_dir}/{query}__vs__{reference}.snpdiffs"
        ...
        [ref_bases,percent_ref_aligned,query_bases,percent_query_aligned] = parseMUmmerReport(mummer_dir,report_header)
        ...
        if percent_ref_aligned < min_cov:
            ...
            raw_snp_count = count_lines(f"{mummer_dir}/{report_header}.snps")
            ...
            if raw_snp_count > 10000:
                ...
                [coords_file,bad_coords_file,merged_ref_bed] = parseMUmmerCoords(mummer_dir,report_header,min_iden,min_len)
                ...
                if coords_file.shape[0] == 0:
                    ...
                    if percent_ref_aligned_filtered < min_cov:
                        ...
                        # Read in SNP file
                        snp_file = parseMUmmerSNPs(mummer_dir,report_header)
        ...
        print(f"MUMmer output for {report_header} converted to {snpdiffs_file}")
    else:
        print(f"Genomes from {report_header}.report cannot be found")
